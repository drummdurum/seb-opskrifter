...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    const project = await Counter.findById(req.params.id);
...
router.put('/:id/details', async (req, res, next) => {
  try {
    ...
    const project = await Counter.findByIdAndUpdate(req.params.id, details, { new: true, runValidators: true });
...
router.post('/:id/notes', async (req, res, next) => {
  try {
    ...
    const project = await Counter.findByIdAndUpdate(
      req.params.id,
      { $push: { notes: { text } } },
...
router.delete('/:id/notes/:noteId', async (req, res, next) => {
  try {
    await Counter.findByIdAndUpdate(req.params.id, { $pull: { notes: { _id: req.params.noteId } } });
...
router.patch('/:id/count', async (req, res, next) => {
  try {
    ...
    const counter = await Counter.findOneAndUpdate(
      { _id: req.params.id, ...(change < 0 ? { count: { $gt: 0 } } : {}) },
      { $inc: { count: change } },
      { new: true, runValidators: true }
    );
    if (!counter) {
...
router.patch('/:id/reset', async (req, res, next) => {
  try {
    if (!mongoose.isValidObjectId(req.params.id)) return res.status(400).json({ error: 'Ugyldigt projekt-id' });
    const counter = await Counter.findByIdAndUpdate(req.params.id, { count: 0 }, { new: true });
...
router.delete('/:id', async (req, res, next) => {
  try {
    if (!mongoose.isValidObjectId(req.params.id)) return res.status(400).send('Ugyldigt projekt-id');
    const project = await Counter.findByIdAndDelete(req.params.id);
    if (project) {
